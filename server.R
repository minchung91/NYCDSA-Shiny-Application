...
shinyServer(function(input, output) {
  # google searches celebrity comparisons plot #####
  # dataframe
  ...
  searches <- df_googlesearches %>%
    ...
    output$googlesearchesPlot <- renderPlotly({
      ggplot(
        searches, aes(x = Week, y = Searches)) + 
  ...
  cardiendorsementss <- df_cardiendorsements %>%
    ...
    output$cardiendorsementsPlot <- renderPlotly({
      ggplot(
        cardiendorsementss, aes(x = Week, y = Searches)) + 
        geom_area(aes(color = SearchTerm, fill = SearchTerm),
                  alpha = 0.5, position = position_dodge(0.8)) +
  ...
  jayendorsementss <- df_jayendorsements %>%
    ...
    output$jayendorsementsPlot <- renderPlotly({
      ggplot(
        jayendorsementss, aes(x = Week, y = Searches)) + 
        geom_area(aes(color = SearchTerm, fill = SearchTerm),
                  alpha = 0.5, position = position_dodge(0.8)) +
  ...
  drakeendorsementss <- df_drakeendorsements %>%
    ...
    output$drakeendorsementsPlot <- renderPlotly({
      ggplot(
        drakeendorsementss, aes(x = Week, y = Searches)) + 
        geom_area(aes(color = SearchTerm, fill = SearchTerm),
                  alpha = 0.5, position = position_dodge(0.8)) +
        scale_color_manual(values = c("#00AFBB", "#E7B800", "#EED5B7", "darksalmon", "darkseagreen2", "lightpink3", "lightcyan3", "navajowhite3", "lightskyblue1")) +
        scale_fill_manual(values = c("#00AFBB", "#E7B800", "#EED5B7", "darksalmon", "darkseagreen2", "lightpink3", "lightcyan3", "navajowhite3", "lightskyblue1")) +
  ...
  kanyeendorsementss <- df_kanyeendorsements %>%
    ...
    output$kanyeendorsementsPlot <- renderPlotly({
      ggplot(
        kanyeendorsementss, aes(x = Week, y = Searches)) + 
        geom_area(aes(color = SearchTerm, fill = SearchTerm),
                  alpha = 0.5, position = position_dodge(0.8)) +
        scale_color_manual(values = c("#00AFBB", "#E7B800", "#EED5B7", "darksalmon", "darkseagreen2", "lightpink3", "lightcyan3", "navajowhite3", "lightskyblue1")) +
        scale_fill_manual(values = c("#00AFBB", "#E7B800", "#EED5B7", "darksalmon", "darkseagreen2", "lightpink3", "lightcyan3", "navajowhite3", "lightskyblue1")) +
  ...
  savageendorsementss <- df_savageendorsements %>%
    ...
    output$savageendorsementsPlot <- renderPlotly({
      ggplot(
        savageendorsementss, aes(x = Week, y = Searches)) + 
        geom_area(aes(color = SearchTerm, fill = SearchTerm),
                  alpha = 0.5, position = position_dodge(0.8)) +
        scale_color_manual(values = c("#00AFBB", "#E7B800", "#EED5B7", "darksalmon", "darkseagreen2", "lightpink3", "lightcyan3", "navajowhite3", "lightskyblue1")) +
        scale_fill_manual(values = c("#00AFBB", "#E7B800", "#EED5B7", "darksalmon", "darkseagreen2", "lightpink3", "lightcyan3", "navajowhite3", "lightskyblue1")) +
    ...
    map <- read.csv(file = "map.csv", head = TRUE, sep = ",")
    head(map)
    # convert csv files to dataframes
    df_map = data.frame(map)
    mapp <- df_map %>%
      select(locationvar, Cardi.B) %>%
      gather(key = "SearchTerm", value = "Percentages", -locationvar)
    output$map <- renderGvis({
      gvisGeoChart(mapp, locationvar = 'locationvar', colorvar = 'Percentages', hovervar = 'Percentages',
                   options=list(region="US", displayMode="regions", resolution="provinces", width="auto", height="auto"))
    })
    mapp1 <- df_map %>%
      select(locationvar, jay.z) %>%
      gather(key = "SearchTerm", value = "Percentages", -locationvar)
    output$map2 <- renderGvis({
      gvisGeoChart(mapp1, locationvar = 'locationvar', colorvar = 'Percentages', hovervar = 'Percentages',
                    options=list(region="US", displayMode="regions", resolution="provinces", width="auto", height="auto"))
    })
    mapp2 <- df_map %>%
      select(locationvar, drake) %>%
      gather(key = "SearchTerm", value = "Percentages", -locationvar)
    output$map3 <- renderGvis({
      gvisGeoChart(mapp2, locationvar = 'locationvar', colorvar = 'Percentages', hovervar = 'Percentages',
                   options=list(region="US", displayMode="regions", resolution="provinces", width="auto", height="auto"))
    })
    mapp3 <- df_map %>%
      select(locationvar, kanye.west) %>%
      gather(key = "SearchTerm", value = "Percentages", -locationvar)
    output$map4 <- renderGvis({
      gvisGeoChart(mapp3, locationvar = 'locationvar', colorvar = 'Percentages', hovervar = 'Percentages',
                   options=list(region="US", displayMode="regions", resolution="provinces", width="auto", height="auto"))
    })
    mapp4 <- df_map %>%
      select(locationvar, X21.savage) %>%
      gather(key = "SearchTerm", value = "Percentages", -locationvar)
    output$map5 <- renderGvis({
      gvisGeoChart(mapp4, locationvar = 'locationvar', colorvar = 'Percentages', hovervar = 'Percentages',
                   options=list(region="US", displayMode="regions", resolution="provinces", width="auto", height="auto"))
    })
})
